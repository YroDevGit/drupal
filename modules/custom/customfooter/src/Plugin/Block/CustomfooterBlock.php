...
/**
 * Provides a 'Customfooter' Block.
 *
 * @Block(
 *   id = "customfooter_block",
 *   admin_label = @Translation("Customfooter Block"),
...
class CustomfooterBlock extends BlockBase {
  ...
  public function blockSubmit($form, FormStateInterface $form_state)
  {
    ...
    foreach ($items as $delta => &$item) {
      if (!empty($item[$childKey])) {
        $file = \Drupalile\Entity\File::load($item[$childKey][0]);
        if ($file) {
          $file->setPermanent();
          $file->save();
          $item[$childKey] = [
  ...
  public function build()
  {
    ...
    $ret =  [
      ...
      '#attached' => [
        'library' => [
          'customfooter/customfooter-styles',
        ],
      ],
    ];
